...
"[project]/src/app/api/stocks/[symbol]/chart/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(req, { params }) {
    try {
        ...
        const startDate = new Date(Date.now() - (range === '5y' ? 5 : range === '3y' ? 3 : range === '10y' ? 10 : 1) * 365 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
        const [prices, corpActions] = await Promise.all([
            adapter.prices.getPrices(stock.id, {
                startDate,
                endDate
            }),
        ...
        try {
            ...
            if (nifty) {
                benchmark = await adapter.prices.getPrices(nifty.id, {
                    startDate,
                    endDate
                });
            }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__68b77b51._.js.map