...
import type {
    StockSummary, PriceBar, CorporateAction, CompanyProfile, CompanyDocument,
    CompanyEvent, QuarterlyResult, BalanceSheet, CashFlow, AnomalyFlag,
    ShareholdingPattern, GovernanceScore, FactorExposure, FactorContext,
    EarningsQuality, ComputedRatios, PeerComparison, FeedItem,
} from "./types";
...
function buildAnnual(quarterly: QuarterlyResult[]): QuarterlyResult[] {
    ...
    return [...byYear.entries()].filter(([, qs]) => qs.length >= 2).map(([yr, qs]) => ({
        periodEnd: `${yr}-03-31`,
        ...
        stocks: {
            async search(query: string, limit = 10): Promise<StockSummary[]> {
                ...
                const rows = await pgDb.queryAll<{ id: string; nse_symbol: string | null; bse_code: string | null; name: string; sector: string | null; industry_group: string | null; industry: string | null; sub_industry: string | null; isin: string | null }>(
                    `SELECT id, nse_symbol, bse_code, name, sector, industry_group, industry, sub_industry, isin FROM assets WHERE is_active = 1 AND (LOWER(nse_symbol) LIKE $1 OR LOWER(name) LIKE $1 OR bse_code LIKE $1) ORDER BY CASE WHEN LOWER(nse_symbol) = LOWER($2) THEN 0 WHEN LOWER(nse_symbol) LIKE LOWER($2)||'%' THEN 1 ELSE 2 END, name LIMIT $3`,
                    [q, query, limit * 2]
                );
                const seen = new Set<string>(); const results: StockSummary[] = [];
                for (const r of rows) {
                    const symbol = r.nse_symbol ?? r.bse_code ?? "";
                    if (!symbol || seen.has(symbol)) continue; seen.add(symbol);
                    results.push({ id: r.id as unknown as number, symbol, name: r.name, exchange: r.nse_symbol ? "NSE" : "BSE", sector: r.sector ?? undefined, industryGroup: r.industry_group ?? undefined, industry: r.industry ?? undefined, subIndustry: r.sub_industry ?? undefined, isin: r.isin ?? undefined, assetClass: "EQUITY" });
            ...
            async getById(id: number): Promise<StockSummary | null> {
                const r = await pgDb.queryOne<{ id: string; nse_symbol: string | null; bse_code: string | null; name: string; sector: string | null }>(
                    `SELECT id, nse_symbol, bse_code, name, sector FROM assets WHERE id = $1`, [String(id)]
                );
                if (!r) return null;
            ...
            async getBySymbol(symbol: string): Promise<StockSummary | null> {
                const r = await pgDb.queryOne<{ id: string; nse_symbol: string | null; bse_code: string | null; name: string; sector: string | null; industry_group: string | null; industry: string | null; sub_industry: string | null; isin: string | null; nse_listed: number; msi_sector: string | null; msi_industry_group: string | null }>(
                    `SELECT id, nse_symbol, bse_code, name, sector, industry_group, industry, sub_industry, isin, nse_listed, msi_sector, msi_industry_group FROM assets WHERE (nse_symbol = $1 OR bse_code = $1) AND is_active = 1 LIMIT 1`,
                    [symbol.toUpperCase()]
                );
                if (!r) return null;
                return { id: r.id as unknown as number, symbol: r.nse_symbol ?? r.bse_code ?? symbol, name: r.name, exchange: r.nse_listed ? "NSE" : "BSE", sector: r.sector ?? r.msi_sector ?? undefined, industryGroup: r.industry_group ?? undefined, industry: r.industry ?? r.msi_industry_group ?? undefined, subIndustry: r.sub_industry ?? undefined, isin: r.isin ?? undefined, assetClass: "EQUITY" };
            },
            async getDetail(symbol: string): Promise<StockDetail | null> {
                const asset = await pgDb.queryOne<{ id: string; nse_symbol: string | null; bse_code: string | null; name: string; sector: string | null; industry_group: string | null; industry: string | null; sub_industry: string | null; isin: string | null; nse_listed: number; msi_sector: string | null; msi_industry_group: string | null; listing_date: string | null; face_value: number | null }>(
                    `SELECT id, nse_symbol, bse_code, name, sector, industry_group, industry, sub_industry, isin, nse_listed, msi_sector, msi_industry_group, listing_date, face_value FROM assets WHERE (nse_symbol = $1 OR bse_code = $1) AND is_active = 1 LIMIT 1`,
                    [symbol.toUpperCase()]
                );
                if (!asset) return null;
                const [lp, range, cr, vol] = await Promise.all([
                    tsDb.queryOne<{ close: number; date: string; prev_close: number | null }>(
                        `SELECT close, date, LAG(close) OVER (ORDER BY date) as prev_close FROM daily_prices WHERE asset_id = $1 AND source_exchange IN ('NSE','BSE') AND close > 0 ORDER BY date DESC LIMIT 1`, [asset.id]
                    ),
                    tsDb.queryOne<{ high52w: number; low52w: number }>(
                        `SELECT MAX(high) as high52w, MIN(low) as low52w FROM daily_prices WHERE asset_id = $1 AND date >= $2 AND source_exchange IN ('NSE','BSE')`, [asset.id, daysAgo(365)]
                    ),
                    pgDb.queryOne<{ market_cap_cr: number | null; pe_ttm: number | null; pb: number | null; dividend_yield: number | null; roe: number | null; roce: number | null; debt_equity: number | null }>(
                        `SELECT market_cap_cr, pe_ttm, pb, dividend_yield, roe, roce, debt_equity FROM computed_ratios WHERE asset_id = $1`, [asset.id]
                    ),
                    tsDb.queryOne<{ volume: number }>(
                        `SELECT volume FROM daily_prices WHERE asset_id = $1 AND source_exchange IN ('NSE','BSE') AND close > 0 ORDER BY date DESC LIMIT 1`, [asset.id]
                    ),
                ]);
                const price = lp?.close ?? 0;
                const pctChange1d = lp?.prev_close && lp.prev_close > 0 ? +((price / lp.prev_close - 1) * 100).toFixed(2) : 0;
                return { id: asset.id as unknown as number, symbol: asset.nse_symbol ?? asset.bse_code ?? symbol, nseSymbol: asset.nse_symbol ?? undefined, bseCode: asset.bse_code ?? undefined, name: asset.name, exchange: asset.nse_listed ? "NSE" : "BSE", sector: asset.sector ?? asset.msi_sector ?? undefined, industryGroup: asset.industry_group ?? undefined, industry: asset.industry ?? asset.msi_industry_group ?? undefined, subIndustry: asset.sub_industry ?? undefined, isin: asset.isin ?? undefined, assetClass: "EQUITY", price, priceDate: lp?.date ?? undefined, pctChange1d, high52w: range?.high52w ?? undefined, low52w: range?.low52w ?? undefined, marketCapCr: cr?.market_cap_cr ?? undefined, pe: cr?.pe_ttm ?? undefined, pb: cr?.pb ?? undefined, dividendYield: cr?.dividend_yield ?? undefined, roe: cr?.roe ?? undefined, roce: cr?.roce ?? undefined, debtEquity: cr?.debt_equity ?? undefined, volume: vol?.volume ?? undefined, faceValue: asset.face_value ?? 1, listedDate: asset.listing_date ?? undefined };
            },
            async getPeers(symbol: string): Promise<PeerComparison[]> {
                const asset = await pgDb.queryOne<{ id: string; sector: string | null; screener_industry_code: string | null; msi_sector: string | null }>(
                    `SELECT id, sector, screener_industry_code, msi_sector FROM assets WHERE (nse_symbol = $1 OR bse_code = $1) AND is_active = 1 LIMIT 1`, [symbol.toUpperCase()]
                );
                if (!asset) return [];
                ...
                const peers = await pgDb.queryAll<{ id: string; nse_symbol: string | null; bse_code: string | null; name: string }>(
                    `SELECT id, nse_symbol, bse_code, name FROM assets WHERE ${field} = $1 AND id != $2 AND is_active = 1 ORDER BY CASE WHEN nse_symbol IS NOT NULL THEN 0 ELSE 1 END, name LIMIT 12`,
                    [val, asset.id]
                );
                const results = await Promise.all(peers.map(async p => {
                    const [lp, cr] = await Promise.all([
                        tsDb.queryOne<{ close: number; date: string }>(`SELECT close, date FROM daily_prices WHERE asset_id = $1 AND source_exchange IN ('NSE','BSE') AND close > 0 ORDER BY date DESC LIMIT 1`, [p.id]),
                        pgDb.queryOne<{ market_cap_cr: number | null; pe_ttm: number | null; pb: number | null; ev_ebitda: number | null; roce: number | null; roe: number | null; debt_equity: number | null; pat_margin: number | null; operating_margin: number | null; revenue_growth_1y: number | null; pat_growth_1y: number | null; dividend_yield: number | null }>(
                            `SELECT market_cap_cr, pe_ttm, pb, ev_ebitda, roce, roe, debt_equity, pat_margin, operating_margin, revenue_growth_1y, pat_growth_1y, dividend_yield FROM computed_ratios WHERE asset_id = $1`, [p.id]
                        ),
                    ]);
                    if (!lp || lp.close <= 0) return null;
                    return { symbol: p.nse_symbol ?? p.bse_code ?? "", nseSymbol: p.nse_symbol ?? undefined, name: p.name, marketCapCr: cr?.market_cap_cr ?? null, peTtm: cr?.pe_ttm ?? null, pb: cr?.pb ?? null, evEbitda: cr?.ev_ebitda ?? null, roce: cr?.roce ?? null, roe: cr?.roe ?? null, debtEquity: cr?.debt_equity ?? null, patMargin: cr?.pat_margin ?? null, operatingMargin: cr?.operating_margin ?? null, price: lp.close, revenueGrowth1y: cr?.revenue_growth_1y ?? null, patGrowth1y: cr?.pat_growth_1y ?? null, dividendYield: cr?.dividend_yield ?? null } as PeerComparison;
                }));
        ...
        prices: {
            async getPrices(assetId: number, opts?: { startDate?: string; endDate?: string; range?: string }): Promise<PriceBar[]> {
                ...
                const rows = await tsDb.queryAll<{ date: string; open: number; high: number; low: number; close: number; volume: number; source_exchange: string }>(
                    `SELECT date, open, high, low, close, volume, source_exchange FROM daily_prices WHERE asset_id = $1 AND date >= $2 AND date <= $3 ORDER BY date ASC`,
        ...
        company: {
            async getProfile(assetId: string): Promise<CompanyProfile> {
                const r = await pgDb.queryOne<{ name: string; description: string | null; website_url: string | null; listing_date: string | null; management_json: string | null; nse_symbol: string | null }>(
                    `SELECT name, description, website_url, listing_date, management_json, nse_symbol FROM assets WHERE id = $1`, [assetId]
                );
                let mgmt: { md?: string; chairman?: string } = {};
                if (r?.management_json) { try { mgmt = JSON.parse(r.management_json); } catch { /* noop */ } }
                return { description: r?.description ?? `${r?.name ?? "Company"} is a listed Indian company.`, descriptionShort: r?.description ?? `${r?.name ?? "Company"} is a listed Indian company.`, founded: r?.listing_date?.slice(0, 4) ?? "N/A", foundedYear: r?.listing_date ? Number(r.listing_date.slice(0, 4)) : null, website: r?.website_url ?? `https://www.${(r?.nse_symbol ?? "company").toLowerCase()}.com`, md: mgmt.md ?? "N/A", chairman: mgmt.chairman ?? "N/A", indexMemberships: [] };
            },
            async getCorporateActions(assetId: string, limit = 20): Promise<CorporateAction[]> {
                const rows = await pgDb.queryAll<{ id: string; action_type: string; ex_date: string; record_date: string | null; dividend_amount: number | null; ratio_numerator: number | null; ratio_denominator: number | null; adjustment_factor: number | null; source_exchange: string | null; raw_announcement: string | null }>(
                    `SELECT id, action_type, ex_date, record_date, dividend_amount, ratio_numerator, ratio_denominator, adjustment_factor, source_exchange, raw_announcement FROM corporate_actions WHERE asset_id = $1 ORDER BY ex_date DESC LIMIT $2`,
                    [assetId, limit]
                );
                return rows.map(r => ({ id: 0, actionType: r.action_type, exDate: r.ex_date, recordDate: r.record_date ?? undefined, dividendAmount: r.dividend_amount ?? undefined }));
            },
            async getEvents(_assetId: string, _limit = 10): Promise<CompanyEvent[]> { return []; },
            async getDocuments(_assetId: string, _docType?: string): Promise<CompanyDocument[]> { return []; },
            async getFinancials(assetId: string, opts?: { consolidated?: boolean }) {
                ...
                const [msiQ, msiBS, msiCF, msiR, scrQ] = await Promise.all([
                    tsDb.queryAll<{ period_end_date: string; revenue_ops: number | null; operating_profit: number | null; finance_costs: number | null; depreciation: number | null; profit_before_tax: number | null; net_profit: number | null; basic_eps: number | null }>(
                        `SELECT period_end_date, revenue_ops, operating_profit, finance_costs, depreciation, profit_before_tax, net_profit, basic_eps FROM ${qT} WHERE asset_id = $1 ORDER BY period_end_date DESC LIMIT 20`, [assetId]
                    ),
                    tsDb.queryAll<{ period_end_date: string; equity_capital: number | null; reserves: number | null; long_term_borrowings: number | null; short_term_borrowings: number | null; total_assets: number | null; cash_equivalents: number | null; fixed_assets: number | null; investments: number | null }>(
                        `SELECT period_end_date, equity_capital, reserves, long_term_borrowings, short_term_borrowings, total_assets, cash_equivalents, fixed_assets, investments FROM ${bsT} WHERE asset_id = $1 ORDER BY period_end_date DESC LIMIT 10`, [assetId]
                    ),
                ...
                const balanceSheet: BalanceSheet[] = msiBS.map(r => { const eq = (r.equity_capital ?? 0) + (r.reserves ?? 0); const debt = (r.long_term_borrowings ?? 0) + (r.short_term_borrowings ?? 0); return { periodEnd: r.period_end_date, totalEquity: eq || null, totalDebt: debt || null, totalAssets: r.total_assets ?? null, cash: r.cash_equivalents ?? null, cashEquivalents: r.cash_equivalents ?? null, fixedAssets: r.fixed_assets ?? null, investments: r.investments ?? null, bookValue: eq || null }; });
                const cashFlow: CashFlow[] = msiCF.map(r => ({ periodEnd: r.period_end_date, operatingCf: r.net_cash_operating ?? undefined, investingCf: r.net_cash_investing ?? undefined, financingCf: r.net_cash_financing ?? undefined, capex: r.capex ?? undefined }));
                const ratios = msiR.map(r => ({ periodEndDate: r.period_end_date, debtorDays: r.debtor_days ?? null, inventoryDays: null, daysPayable: null, roce: r.roce ?? null, operatingMargin: r.ebit_margin ?? null, patMargin: r.net_profit_margin ?? null }));
            ...
            async getAnalytics(assetId: string) {
                const [lp, cr, msi] = await Promise.all([
                    tsDb.queryOne<{ close: number }>(`SELECT close FROM daily_prices WHERE asset_id = $1 AND source_exchange IN ('NSE','BSE') AND close > 0 ORDER BY date DESC LIMIT 1`, [assetId]),
                    pgDb.queryOne<{ market_cap_cr: number | null; pe_ttm: number | null; pb: number | null; ev_ebitda: number | null; roce: number | null; roe: number | null; debt_equity: number | null; pat_margin: number | null; operating_margin: number | null; revenue_growth_1y: number | null; pat_growth_1y: number | null; dividend_yield: number | null; quality_score: number | null }>(
                        `SELECT market_cap_cr, pe_ttm, pb, ev_ebitda, roce, roe, debt_equity, pat_margin, operating_margin, revenue_growth_1y, pat_growth_1y, dividend_yield, quality_score FROM computed_ratios WHERE asset_id = $1`, [assetId]
                    ),
                    pgDb.queryOne<{ composite_rating: number | null; pct_from_high: number | null }>(
                        `SELECT composite_rating, pct_from_high FROM msi_company_data WHERE asset_id = $1`, [assetId]
                    ),
                ]);
                const price = lp?.close ?? 0;
                return {
                    ...
                    ratios: { peTtm: cr?.pe_ttm ?? undefined, pb: cr?.pb ?? undefined, evEbitda: cr?.ev_ebitda ?? undefined, marketCapCr: cr?.market_cap_cr ?? undefined, price, pctFrom52wHigh: msi?.pct_from_high ?? undefined, roe: cr?.roe ?? undefined, roce: cr?.roce ?? undefined, debtEquity: cr?.debt_equity ?? undefined, dividendYield: cr?.dividend_yield ?? undefined, patMargin: cr?.pat_margin ?? undefined, operatingMargin: cr?.operating_margin ?? undefined, revenueGrowth1y: cr?.revenue_growth_1y ?? undefined, patGrowth1y: cr?.pat_growth_1y ?? undefined, qualityScore: cr?.quality_score ?? undefined } as ComputedRatios,
                };
            },
        },

        follow: {
            async getStatus(_userId: string, _symbol: string) { return { isFollowing: false, followerCount: 0, alertConfig: undefined }; },
            async follow() { },
            async unfollow() { },
        },

        feed: {
            async getUserFeed(_userId: string, _limit = 50, _offset = 0): Promise<FeedItem[]> { return []; },
